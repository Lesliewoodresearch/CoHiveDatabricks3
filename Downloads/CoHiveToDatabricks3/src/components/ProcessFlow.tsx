...
const processSteps: ProcessStep[] = [
  ...
  // Row 2 - Research
  { id: 'research', label: 'Knowledge Base', color: stepColors.research,  position: { x: 0, y: 570 }, row: 2 },
  
  // Row 3 - Middle cluster (top row of cluster)
  { id: 'Luminaries', label: 'Luminaries', color: stepColors.Luminaries,  position: { x: 225, y: 140 }, row: 3 }, 
  { id: 'panelist', label: 'Panelist', color: stepColors.panelist,  position: { x: 120, y: 305 }, row: 5 }, 
  { id: 'Consumers', label: 'Consumers', color: stepColors.Consumers,  position: { x: 120, y: 195 }, row: 4 }, 
  { id: 'competitors', label: 'Competitors', color: stepColors.competitors,  position: { x: 15, y: 250 }, row: 4 },
  { id: 'Colleagues', label: 'Colleagues', color: stepColors.Colleagues,  position: { x: 120, y: 85 }, row: 3 },
  { id: 'cultural', label: 'Cultural\nVoices', color: stepColors.cultural,  position: { x: 225, y: 250 }, row: 4 },
  { id: 'social', label: 'Social\nListening', color: stepColors.social,  position: { x: 15, y: 140 }, row: 3 },
  
  { id: 'Wisdom', label: 'Share Your\nWisdom', color: stepColors.Wisdom,  position: { x: 120, y: 570 }, row: 6 },
  { id: 'Grade', label: 'Score\nResults', color: stepColors.Grade,  position: { x: 220, y: 395 }, row: 7 },
  { id: 'Findings', label: 'Findings', color: stepColors.Findings,  position: { x: 75, y: 445 }, row: 8 },
  { id: 'review', label: 'My\nFiles', color: stepColors.review,  position: { x: 240, y: 570 }, row: 9 }
...
const hexInfo: { [key: string]: { title: string; description: string; details: string[] } } = {
  Enter: {
    title: 'Enter',
    description: 'The starting point for every CoHive project. This hexagon initializes your knowledge workflow.',
    details: [
  ...
  research: {
    ...
    description: 'Central hub for managing all knowledge assets, synthesis, and persona files.',
    details: [
  ...
  Wisdom: {
    title: 'Wisdom',
    description: 'Add your insights about brands, markets, flavors, consumers and any other insights that would help build a better foundation for results',
    details: [
...
export function ProcessFlow({ activeStep, onStepChange, completedSteps = [], isEnterComplete = false, userRole, hexExecutions = {}, projectType }: ProcessFlowProps) {
  ...
  const isStepClickable = (stepId: string): boolean => {
    ...
    if (projectType === 'War Games') {
      const disabledHexes = ['Luminaries', 'panelist', 'Colleagues', 'Consumers', 'cultural', 'social', 'Grade'];
 ...
 //     }
  ...
  return (
    <div className="p-2" style={{ height: '750px' }}>
      <div className="relative" style={{ minWidth: '360px', width: '360px', height: '750px' }}>
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 0 }}>
          ...
          */}
          {/* Enter to Colleagues - Curved Arrow 
          <path 
            d="M 60 105 Q 90 130, 120 140" 
            stroke="#9333EA" 
            strokeWidth="3" 
            fill="none" 
          ...
          */}
          ...
          {/* Red Triangle - flat side towards Enter, point towards (125, 125)  77,120 115,100 120,145 -- "75,131 110,100 120,145""  */}
          <polygon 
            points="77,120 120,98 120,145" 
            fill="#FF0000" 
          />
         ...
         */}
             {/* S-shaped curved arrow to Grade 
          <path 
            d="M 250 355 C 285 375, 360 355, 335 415" 
            stroke="#9333EA" 
            strokeWidth="3" 
            fill="none" 
            markerEnd="url(#arrowhead)" 
          ...
          */}
           ...
           {/* Red Triangle - flat side towards Enter, point towards (125, 125)   245,375 275,355 285,390  -- 240,375 275,345 285,390*/}
          <polygon 
            points="240,382 275,352 285,397" 
            fill="#FF0000" 
          />
          ...
          {/* Grade to Findings 
          <line x1="235" y1="460" x2="195" y2="470" stroke="#9333EA" strokeWidth="3" markerEnd="url(#arrowhead)" />
*/}
           {/* Red Triangle - flat side towards Enter, point towards (125, 125)  240,480 220,445 200,475 */}
          <polygon 
            points="197,492 222,452 245,495" 
            fill="#FF0000" 
          />
