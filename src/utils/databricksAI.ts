...
/**
 * Execute an AI prompt with streaming support
 * Calls onChunk for each piece of text as it arrives
 */
export async function executeAIPromptStreaming(
  params: AIPromptParams,
  onChunk: (chunk: string) => void
): Promise<AIPromptResponse> {
  try {
    console.log('🤖 Executing AI Prompt (Streaming):', params.prompt.substring(0, 100) + '...');
    ...
    // If response doesn't support streaming, fall back to regular response
    ...
    return {
      ...
      model: 'streaming',
      usage: { promptTokens: 0, completionTokens: 0, totalTokens: 0 },
      metadata: { kbContextUsed: false, conversationLength: params.conversationHistory?.length || 0 },
    };
...
export class AIConversation {
  ...
  async askStreaming(prompt: string, onChunk: (chunk: string) => void): Promise<string> {
    const result = await executeAIPromptStreaming({
      prompt,
      userEmail: this.userEmail,
      userRole: this.userRole,
      conversationHistory: this.history,
