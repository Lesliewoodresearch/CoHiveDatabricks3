...
/**
 * Generate an AI summary from Findings data
 */
export async function generateSummary(params: {
  ...
  try {
    console.log('🤖 Generating AI summary:', params.fileName);
    ...
    const response = await fetch('/api/databricks/ai/summarize', {
      ...
      body: JSON.stringify({
        ...
        workspaceHost: auth.workspaceHost,
      }),
    });

    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.error || `Summary generation failed: ${response.statusText}`);
    }
