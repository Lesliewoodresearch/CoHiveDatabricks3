...
Start by introducing yourself briefly (1 sentence) and then immediately ask your first insightful question.`;
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const placeholderIndex = messages.length + 1; // +1 because we already added user message
      setMessages(prev => [...prev, { role: 'assistant', content: '' }]);

      // Stream the response
      let streamedContent = '';
      await conversation.askStreaming(promptText, (chunk: string) => {
      ...
      setMessages(prev => {
        const newMessages = [...prev];
        // Replace the last (streaming) message with error
