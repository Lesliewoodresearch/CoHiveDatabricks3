...
import { X, CircleCheck, Loader, ChevronDown, ChevronUp, CircleAlert } from 'lucide-react';
...
export function AssessmentModal({
  ...
  // BJS: Summary tab — populated by the neutral summarizer pass in run.js
  ...
  const getModelDisplayName = (modelId: string): string => {
    ...
    return modelId.replace('databricks-', '').split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
  };
  ...
  const runAssessment = useCallback(async () => {
    ...
    setIsComplete(false);
    setCitedFiles([]);
    // BJS: reset summary state on each new run
    setSummary(null);
    setActiveTab('rounds');
    ...
    try {
      ...
      const response = await fetch('/api/databricks/assessment/run', {
        ...
        body: JSON.stringify({
          ...
          // BJS: Pass conversation settings to API
          conversationMode,
          modelEndpoint,
        }),
      });
      ...
      // BJS: Store summary from summarizer pass; auto-switch to summary tab if present
      if (result.summary) {
  ...
  useEffect(() => {
    const handleSelectionChange = () => {
     ...
     /* Commenting this out since we want to have GEMs in the summary
        // BJS: No gem saving on the summary tab — summary doesn't reference raw KB files
  ...
  const handleMouseUp = useCallback((_e: React.MouseEvent) => {
    ...
    /* Commenting out so summaries can have GEMs
    // BJS: No gem saving from summary tab
  ...
  const formatContent = (content: string): React.ReactNode => {
    ...
    return lines.map((line, i) => {
      const personaMatch = line.match(/^###\s+(.+)$/) || line.match(/^\*\*([^*]+):\*\*\s*(.*)$/);
      if (personaMatch) {
        ...
        return (
          <div key={i} className="mb-3">
            ...
            {rest && <p className="text-gray-800 text-sm leading-relaxed">{formatCitations(rest)}</p>}
          </div>
        );
      }
      const boldHeadingMatch = line.match(/^\*\*([^*]+):\*\*$/);
      if (boldHeadingMatch) return <h4 key={i} className="font-semibold text-gray-900 text-sm mt-3 mb-1">{boldHeadingMatch[1]}:</h4>;
      if (line.trim().startsWith('- ') || line.trim().startsWith('* ')) {
        const bulletText = line.trim().substring(2);
        return (
          <div key={i} className="flex items-start gap-2 ml-4 mb-1">
            <span className="text-gray-400 text-xs mt-0.5">•</span>
            <p className="text-gray-700 text-sm leading-relaxed flex-1">{formatCitations(bulletText)}</p>
          </div>
        );
      }
  ...
  // BJS: Lightweight markdown renderer for the summary panel.
  // Handles headers, bullets, bold — no citation highlighting (summary is neutral).
  const formatSummary = (text: string): React.ReactNode => {
    const lines = text.split('\n');
    return lines.map((line, i) => {
      if (line.startsWith('## ')) return <h3 key={i} className="text-gray-900 font-semibold text-base mt-4 mb-2">{line.slice(3)}</h3>;
      if (line.startsWith('### ')) return <h4 key={i} className="text-gray-800 font-semibold text-sm mt-3 mb-1">{line.slice(4)}</h4>;
      if (line.startsWith('# ')) return <h2 key={i} className="text-gray-900 font-bold text-lg mt-4 mb-2">{line.slice(2)}</h2>;
      if (line.trim().startsWith('- ') || line.trim().startsWith('* ')) {
        const bulletText = line.trim().substring(2);
    ...
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-8" style={{ backgroundColor: 'rgba(0, 0, 0, 0.2)' }}>
        <div className="bg-white rounded-xl shadow-2xl flex flex-col" style={{ width: '75%', height: '75vh', maxWidth: '1200px' }}>
          ...
          <div className="bg-white border-b-2 border-gray-200 px-6 py-4 flex items-center justify-between flex-shrink-0 rounded-t-xl">
            <div className="flex items-center gap-4">
              <div>
                ...
                <p className="text-purple-600 text-xs font-medium mt-0.5 flex items-center gap-1">
                  <span className="inline-block w-1.5 h-1.5 bg-purple-600 rounded-full"></span>
                  Model: {getModelDisplayName(modelEndpoint)}
                </p>
              </div>
          ...
          {/* BJS: Tab bar — only shown once assessment is complete */}
          {isComplete && (
          ...
          {/* Gem instructions — only on rounds tab */}
          {isComplete && (
          ...
          <div ref={contentRef} className="flex-1 overflow-y-auto bg-gray-50" style={{ position: 'relative' }}>
                ...
                {summary ? (
                  <div className="bg-white border-2 border-green-200 rounded-lg p-6">
                    <div className="flex items-center gap-2 mb-4 pb-3 border-b border-green-100">
                      ...
                      <span className="text-green-600 text-xs ml-auto">Generated by neutral summarizer · not a persona</span>
                    </div>
            ...
            {/* Rounds tab content */}
            {activeTab === 'rounds' && (
              <div className="p-6 space-y-4 max-w-4xl mx-auto" onMouseUp={handleMouseUp} onMouseDown={handleMouseDown}>
            ...
            {/* Floating gem button — rounds tab only */}
            {floatingBtn && (
              <div
          ...
          {isComplete && (
            <div className="bg-white border-t-2 border-gray-200 px-6 py-4 flex items-center justify-end flex-shrink-0">
              <button 
                onClick={() => {
                  // Pass results to parent before closing
                  if (onAcceptResults) {
                    onAcceptResults({
                      rounds,
                      citedFiles,
      ...
      {/* Toasts - Outside modal for proper z-index layering */}
