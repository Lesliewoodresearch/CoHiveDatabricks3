...
import { uploadToKnowledgeBase, downloadFile, listKnowledgeBaseFiles, type KnowledgeBaseFile, generateSummary } from '../utils/databricksAPI';
...
export default function ProcessWireframe() {
  ...
  // Last assessment results - carry forward to next hex until iteration is saved
  ...
  // Session versioning state for tracking iterations/summaries
  ...
  useEffect(() => {
    ...
    if (savedTemplates) {
      try {
        ...
        // Migration: Add Data Scientist template if it doesn't exist
        const hasDataScientist = templates.some((t: UserTemplate) => t.id === 'data-scientist');
        if (!hasDataScientist) {
          const dataScientistTemplate = defaultTemplates.find(t => t.id === 'data-scientist');
          if (dataScientistTemplate) {
            const updatedTemplates = [...templates, dataScientistTemplate];
            setTemplates(updatedTemplates);
            localStorage.setItem('cohive_templates', JSON.stringify(updatedTemplates));
            console.log('✅ Migrated: Added Data Scientist template to existing templates');
          } else {
    ...
    // Load model templates or initialize with defaults
    const savedModelTemplates = localStorage.getItem('cohive_model_templates');
    if (savedModelTemplates) {
      try {
        const modelTemplates = JSON.parse(savedModelTemplates);
        setModelTemplates(modelTemplates);
      } catch (e) {
        console.error('Failed to load saved model templates', e);
        // Initialize with default model templates
        setModelTemplates(defaultModelTemplates);
        localStorage.setItem('cohive_model_templates', JSON.stringify(defaultModelTemplates));
      }
    } else {
      // Initialize with default model templates
      setModelTemplates(defaultModelTemplates);
    ...
    // Load current model template selection or set default
    const savedCurrentModelTemplateId = localStorage.getItem('cohive_current_model_template_id');
    if (savedCurrentModelTemplateId) {
      try {
        setCurrentModelTemplateId(savedCurrentModelTemplateId);
      } catch (e) {
        console.error('Failed to load current model template', e);
        // Set default model template
        setCurrentModelTemplateId('default-claude');
        localStorage.setItem('cohive_current_model_template_id', 'default-claude');
      }
    } else {
      // Set default model template
      setCurrentModelTemplateId('default-claude');
  ...
  // Save session versions to localStorage whenever they change
  useEffect(() => {
    if (Object.keys(sessionVersions).length > 0) {
      saveSessionVersions(sessionVersions);
    }
  }, [sessionVersions]);
  ...
  // Sync currentModelTemplate when currentModelTemplateId or modelTemplates change
  useEffect(() => {
  ...
  const getApprovedResearchFiles = (brand: string, projectType: string): ResearchFile[] => {
    ...
    return researchFiles.filter(file => {
      ...
      // 1. Exact brand + project type match (scope: 'brand')
      // 2. Category files that match the project type (scope: 'category')
      // 3. General files (scope: 'general')
      
      const fileBrand = file.brand?.toLowerCase() || '';
      const fileProjectType = file.projectType?.toLowerCase() || '';
      const userBrand = brand.toLowerCase();
      const userProjectType = projectType.toLowerCase();
      
      // Scope-based filtering
      if (file.scope === 'general') {
        // General files are available to everyone
        return true;
      } else if (file.scope === 'category') {
        // Category files match by project type (category)
        return fileProjectType === userProjectType;
      } else {
        // Brand-specific files (scope: 'brand' or undefined for legacy files)
        return fileBrand === userBrand && fileProjectType === userProjectType;
      }
    });
  };
  ...
  const handleCentralHexExecute = (selectedFiles: string[], assessmentType: string[], assessment: string) => {
    ...
    // For competitors hex, selectedFiles contains competitor data, so use selectedResearchFiles too.
    ...
    console.log(`[${activeStepId}] selectedResearchFiles: ${JSON.stringify(selectedResearchFiles)}`);
    console.log(`[${activeStepId}] selectedFiles: ${JSON.stringify(selectedFiles)}`);
    ...
    // BJS: Get conversation settings from current template (defaults if not set)
    const conversationMode = currentTemplate?.conversationSettings?.conversationMode || 'multi-round';
    ...
    setAssessmentModalProps({
      ...
      brand,
      projectType,
      // BJS: Pass conversation settings to assessment API
      conversationMode,
      modelEndpoint,
    });
  ...
  const handleModelTemplateChange = (templateId: string) => {
    ...
    if (newModelTemplate) {
      setCurrentModelTemplateId(templateId);
      localStorage.setItem('cohive_current_model_template_id', templateId);
    }
  };

  const handleModelTemplateUpdate = (template: ModelTemplate) => {
    const updatedTemplates = modelTemplates.map(t => 
      t.id === template.id ? template : t
    );
    setModelTemplates(updatedTemplates);
    if (currentModelTemplateId === template.id) {
      setCurrentModelTemplateId(template.id);
      localStorage.setItem('cohive_current_model_template_id', template.id);
    }
  ...
  const loadKnowledgeBaseFiles = async () => {
    try {
      ...
      const isResearcher = currentTemplate.role === 'research-analyst' || currentTemplate.role === 'research-leader' || currentTemplate.role === 'data-scientist';
      ...
      console.log(`✅ Found ${kbFiles.length} knowledge base files (all statuses, all users can view)`);

      // FILTER OUT "Findings" files - they should only appear in My Files (ReviewView), not in Knowledge Base
      const filteredFiles = kbFiles.filter(file => file.fileType !== 'Findings');
      console.log(`📚 After filtering out Findings files: ${filteredFiles.length} files remain for Knowledge Base`);
        ...
        <div className="flex-shrink-0">
          ...
          <div className="flex flex-col gap-3 mt-4">
            ...
            <div className="relative">
              ...
              {/* Model Template Manager Button */}
              <button 
                className="w-full px-4 py-2 border-2 border-gray-400 text-gray-700 rounded flex items-center gap-2 hover:bg-gray-50"
                onClick={() => setShowModelTemplateManager(true)}
              >
                <Cpu className="w-4 h-4" />
                Model Templates
              </button>

              {/* Model Template Manager Modal */}
              {showModelTemplateManager && currentModelTemplate && (
                <ModelTemplateManager
                  currentTemplate={currentModelTemplate}
                  availableTemplates={modelTemplates}
                  onTemplateChange={handleModelTemplateChange}
                  onTemplateUpdate={handleModelTemplateUpdate}
                  onTemplateCreate={handleModelTemplateCreate}
                />
              )}
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                <CentralHexView
                  ...
                  lastResults={lastAssessmentResults}
                  conversationMode={currentTemplate?.conversationSettings?.conversationMode || 'multi-round'}
                  modelEndpoint={currentTemplate?.conversationSettings?.modelEndpoint || 'databricks-claude-haiku-4-5'}
                />
              ) : (
                <>
                                ...
                                <input
                                  ...
                                  onChange={async (e) => {
                                    ...
                                    if (e.target.value === 'Save Iteration' && brand && projectType) {
                                      ...
                                      // Generate versioned filename for this iteration
                                      const { fileName: versionedFileName, updatedSession } = generateIterationFileName(
                                        sessionVersions,
                                        brand,
                                        projectType
                                      );
                                      
                                      // Update session versions state
                                      setSessionVersions(prev => ({
                                        ...prev,
                                        [updatedSession.sessionKey]: updatedSession,
                                      }));
                                      ...
                                      const file = createFileFromBlob(blob, versionedFileName);
                                      ...
                                      if (result.success) {
                                        console.log('✅ Iteration saved to Databricks Knowledge Base:', versionedFileName);
                                        alert(`✅ Upload successful! Iteration "${versionedFileName}" saved to the Knowledge Base.`);
                                        ...
                                        // Clear last assessment results for next iteration
                          ...
                          <div className="space-y-1">
                            <label className="flex items-center gap-2 cursor-pointer">
                              <input
                                ...
                                name="saveOrDownload"
                                value="Read"
                                checked={responses[activeStepId]?.[idx] === 'Read'}
                                onChange={async (e) => {
                                  handleResponseChange(idx, e.target.value);
                                  // Generate and show AI summary in markdown format
                                    ...
                                    if (summaryFileName) {
                                      ...
                                      try {
                                        const result = await generateSummary({
                                          ...
                                          userEmail,
                                          userRole,
                                          modelEndpoint: currentTemplate?.conversationMode === 'incremental' 
                                            ? currentTemplate?.modelEndpoint 
                              ...
                              />
                              <span className="text-gray-700">
                                {isGeneratingSummary ? 'Generating Summary...' : 'Read'}
                              </span>
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer">
                              <input
      ...
      {assessmentModalOpen && assessmentModalProps && (
        <AssessmentModal
          ...
          onAcceptResults={(results) => {
            // Store results to carry forward to next hex
          ...
          conversationMode={assessmentModalProps.conversationMode}
